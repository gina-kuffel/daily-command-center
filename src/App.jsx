...
async function addWorkTodoAPI(payload) {
  try {
    const r = await fetch('/api/work-todos?op=add', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (!r.ok) { const e = await r.json().catch(() => ({})); return { success: false, error: e.error || r.statusText }; }
    return { success: true, todo: (await r.json()).todo };
...
const AddTodoInline = ({ defaultName, label = 'Personal To-Do', accentColor = '#3b82f6', onSave, onCancel }) => {
  ...
  const ib = { padding: '7px 10px', borderRadius: '6px', border: '1px solid #e2e8f0', fontSize: '12px', outline: 'none', fontFamily: 'inherit', background: '#fff' };
  ...
  return (
    <div style={{ margin: '6px 0 4px 0', padding: '10px 12px', borderRadius: '8px',
      ...
      <div style={{ display: 'flex', gap: '6px' }}>
        <button onClick={() => onSave(name.trim(), due || null, priority || null)} disabled={!name.trim()}
          style={{ padding: '6px 14px', borderRadius: '6px', fontSize: '12px', fontWeight: 700,
            background: `linear-gradient(135deg, ${accentColor} 0%, #6366f1 100%)`, color: '#fff', border: 'none',
            cursor: name.trim() ? 'pointer' : 'not-allowed', opacity: name.trim() ? 1 : 0.5 }}>Save to {label}</button>
        <button onClick={onCancel} style={{ padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 600,
          background: 'transparent', color: '#64748b', border: '1px solid #e2e8f0', cursor: 'pointer' }}>Cancel</button>
      </div>
    </div>
  );
};
...
const SyncBadge = ({ status }) => {
  ...
  const c = { syncing: { bg: '#eff6ff', text: '#1d4ed8', label: '⟳ Syncing…' }, success: { bg: '#f0fdf4', text: '#15803d', label: '✓ Synced' }, error: { bg: '#fef2f2', text: '#991b1b', label: '✗ Error' } }[status];
  return <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '4px', background: c.bg, color: c.text, marginLeft: '8px', display: 'inline-block' }}>{c.label}</span>;
};

const Badge = ({ label, config }) => (
  <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 600,
    letterSpacing: '0.04em', background: config.bg, color: config.text, border: `1px solid ${config.border}`, textTransform: 'uppercase' }}>{label}</span>
);
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '8px 10px', borderRadius: '8px', marginBottom: '4px',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
          <p style={{ margin: 0, fontSize: '12px', lineHeight: '1.4', color: checked ? '#94a3b8' : '#334155', textDecoration: checked ? 'line-through' : 'none' }}>{task.name}</p>
...
const SlackMentionRow = ({ mention, addingWorkTodoFor, onOpenWorkTodo, onSaveWorkTodo, onCancelWorkTodo }) => {
  ...
  const time = new Date(parseFloat(mention.ts) * 1000).toLocaleTimeString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
  const rowId = `slack_${mention.ts}`;
  const snippet = cleanText.length > 80 ? cleanText.slice(0, 80) + '…' : cleanText;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '8px 10px', borderRadius: '8px', marginBottom: '4px', background: '#fafafa', border: '1px solid #f1f5f9' }}>
...
const GmailRow = ({ email, addingTodoFor, onOpenTodo, onSaveTodo, onCancelTodo, onDismiss }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '8px 10px', borderRadius: '8px', marginBottom: '4px',
      background: email.isActioned ? '#fffbeb' : '#fafafa', border: email.isActioned ? '1px solid #fde68a' : '1px solid #f1f5f9' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
        ...
        {!isOpen && (
          <>
            <button onClick={() => onOpenTodo(rowId)} style={{ fontSize: '11px', fontWeight: 700, color: '#6366f1', background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: '5px', padding: '2px 8px', cursor: 'pointer', marginLeft: 'auto' }}>➕ To-Do</button>
            <button onClick={onDismiss} style={{ fontSize: '12px', fontWeight: 700, color: '#94a3b8', background: 'transparent', border: '1px solid #e2e8f0', borderRadius: '5px', padding: '2px 7px', cursor: 'pointer', lineHeight: 1 }}>×</button>
          </>
        )}
      </div>
...
const CalendarEventRow = ({ event, addingTodoFor, onOpenTodo, onSaveTodo, onCancelTodo }) => {
  ...
  const isOpen = addingTodoFor === rowId;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', padding: '8px 10px', borderRadius: '8px', marginBottom: '4px',
      background: event.needsPrep ? '#fffbeb' : '#fafafa', border: event.needsPrep ? '1px solid #fde68a' : '1px solid #f1f5f9' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        <span style={{ fontSize: '11px', fontWeight: 700, color: '#0369a1' }}>{event.timeLabel}</span>
        {event.isHoliday && <span style={{ fontSize: '10px', background: '#fdf4ff', color: '#7e22ce', border: '1px solid #e9d5ff', borderRadius: '4px', padding: '1px 6px', fontWeight: 600 }}>🎉 Holiday</span>}
        {event.needsPrep && !event.isHoliday && <span style={{ fontSize: '10px', background: '#fef3c7', color: '#92400e', border: '1px solid #fde68a', borderRadius: '4px', padding: '1px 6px', fontWeight: 600 }}>⏰ Reminder</span>}
      </div>
...
const GroceryItem = ({ item, checked, onToggle, onDelete }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 10px', borderRadius: '8px', background: '#fafafa', marginBottom: '4px', border: '1px solid #f1f5f9' }}>
    <input type="checkbox" checked={checked} onChange={() => onToggle(item.id)} style={{ cursor: 'pointer', accentColor: '#10b981' }} />
    <span style={{ flex: 1, fontSize: '13px', color: checked ? '#94a3b8' : '#334155', textDecoration: checked ? 'line-through' : 'none' }}>{item.name}</span>
...
const TodoItem = ({ item, onToggle, onDelete, syncStatus, sourceConfig, accentColor = '#8b5cf6' }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '8px 10px', borderRadius: '8px', marginBottom: '4px',
      ...
      <input type="checkbox" checked={item.completed} onChange={() => onToggle(item.id)} disabled={syncStatus === 'syncing'} style={{ marginTop: '2px', cursor: 'pointer', accentColor }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '2px' }}>
          {item.priority && <Badge label={todoPriorityConfig[item.priority].label} config={todoPriorityConfig[item.priority]} />}
          {item.source && item.source !== 'manual' && src && <span title={item.sourceRef || src.label} style={{ fontSize: '10px', color: src.color, fontWeight: 600 }}>{src.emoji} {src.label}</span>}
          <p style={{ margin: 0, fontSize: '12px', lineHeight: '1.4', color: item.completed ? '#94a3b8' : '#334155', textDecoration: item.completed ? 'line-through' : 'none' }}>{item.name}</p>
          <SyncBadge status={syncStatus} />
        </div>
        {item.due && <span style={{ fontSize: '10px', display: 'block', color: isOverdue ? '#dc2626' : '#64748b', fontWeight: isOverdue ? 700 : 400 }}>{isOverdue ? '⚠ OVERDUE — ' : 'Due '}{item.due}</span>}
      </div>
      <button onClick={() => onDelete(item.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1', fontSize: '14px', lineHeight: 1, padding: '2px', marginTop: '1px' }}>×</button>
    </div>
  );
};
...
export default function DailyCommandCenter() {
  ...
  const handleAddTodo = async () => {
    ...
    const opt = { id: `opt_${Date.now()}`, name: newTodo.trim(), due: newTodoDue || null, priority: newTodoPri || null, source: 'manual', completed: false, createdAt: new Date().toISOString() };
    setTodos(p => [opt, ...p]); setNewTodo(''); setNewTodoDue(''); setNewTodoPri('');
    const r = await addTodoAPI({ name: opt.name, due: opt.due, priority: opt.priority });
    if (r.success) setTodos(p => p.map(t => t.id === opt.id ? r.todo : t));
    else { setTodos(p => p.filter(t => t.id !== opt.id)); alert(`Could not save todo: ${r.error}`); }
  };
  const handleToggleTodo = async (id) => {
    setTodos(p => p.map(t => t.id === id ? { ...t, completed: !t.completed } : t));
    setTodoSyncStatus(p => ({ ...p, [id]: 'syncing' }));
    const r = await toggleTodoAPI(id);
    setTodoSyncStatus(p => ({ ...p, [id]: r.success ? 'success' : 'error' }));
    if (r.success) setTodos(p => p.map(t => t.id === id ? r.todo : t));
  ...
  const handleAddWorkTodo = async () => {
    ...
    const opt = { id: `optw_${Date.now()}`, name: newWorkTodo.trim(), due: newWorkTodoDue || null, priority: newWorkTodoPri || null, source: 'manual', completed: false, createdAt: new Date().toISOString() };
    setWorkTodos(p => [opt, ...p]); setNewWorkTodo(''); setNewWorkTodoDue(''); setNewWorkTodoPri('');
    const r = await addWorkTodoAPI({ name: opt.name, due: opt.due, priority: opt.priority });
    if (r.success) setWorkTodos(p => p.map(t => t.id === opt.id ? r.todo : t));
    else { setWorkTodos(p => p.filter(t => t.id !== opt.id)); alert(`Could not save work todo: ${r.error}`); }
  };
  const handleToggleWorkTodo = async (id) => {
    ...
    const r = await toggleWorkTodoAPI(id);
    setWorkTodoSyncStatus(p => ({ ...p, [id]: r.success ? 'success' : 'error' }));
    if (r.success) setWorkTodos(p => p.map(t => t.id === id ? r.todo : t));
  ...
  const views = [
    { id: 'briefing', label: 'Briefing', icon: '◉' }, { id: 'jira', label: 'Jira', icon: '⊞' },
    { id: 'asana', label: 'Asana', icon: '◎' }, { id: 'work', label: 'Work', icon: '💼' },
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: "'DM Sans', 'Segoe UI', system-ui, sans-serif", background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)' }}>
      ...
      {/* Banner */}
      <div style={{ width: '100%', position: 'relative', overflow: 'hidden', backgroundImage: 'url(https://images.unsplash.com/photo-1514565131-fce0801e5785?q=80&w=2069&auto=format&fit=crop)', backgroundSize: 'cover', backgroundPosition: 'center 60%' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(15,23,42,0.85) 0%, rgba(88,28,135,0.80) 50%, rgba(49,46,129,0.85) 100%)' }} />
        <div style={{ position: 'absolute', top: '16px', right: '48px', zIndex: 2, display: 'inline-flex', alignItems: 'center', gap: '6px', background: syncBg, border: `1px solid ${syncBorder}`, borderRadius: '8px', padding: '6px 12px' }}>
          <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: syncColor, display: 'inline-block' }} />
          <span style={{ color: syncColor, fontSize: '11px', fontWeight: 700 }}>{syncLabel}</span>
        </div>
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '48px 48px 28px', position: 'relative', zIndex: 1, display: 'flex', alignItems: 'flex-end', gap: '16px' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #3b82f6 0%, #6366f1 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '4px solid rgba(255,255,255,0.2)', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
            <span style={{ color: '#fff', fontSize: '32px', fontWeight: 700, fontFamily: "'DM Sans', sans-serif", lineHeight: 1 }}>G</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
            <h1 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '36px', fontWeight: 300, color: '#fff', margin: 0, lineHeight: 1.2 }}><strong style={{ fontWeight: 700 }}>Unit</strong> Actions</h1>
            <p style={{ margin: 0, fontFamily: "'DM Sans', sans-serif", fontSize: '14px', fontWeight: 400, color: '#bfdbfe', letterSpacing: '0.05em' }}>Daily Command Center</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '24px 16px 16px' }}>
        <div style={{ marginBottom: '20px' }}>
          <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '28px', color: '#fff', margin: 0, lineHeight: 1.2 }}>{greeting}, Gina. {greetingEmoji}</h1>
        ...
        <div style={{ display: 'flex', gap: '4px', background: 'rgba(30,41,59,0.5)', borderRadius: '10px', padding: '4px', border: '1px solid rgba(71,85,105,0.5)' }}>
          {views.map(v => (
            <button key={v.id} onClick={() => setActiveView(v.id)} style={{ flex: 1, padding: '10px 4px', borderRadius: '7px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', transition: 'all 0.15s', background: activeView === v.id ? '#fff' : 'transparent', color: activeView === v.id ? '#0f172a' : '#94a3b8' }}>
        ...
        {activeView === 'briefing' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
              ...
              <div style={{ padding: '0 16px 16px' }}>
                {todosLoading ? <LoadingRows message="Loading to-dos…" /> : todosError === 'kv_missing' ? <LoadingRows message="⚠ Vercel KV not set up yet." color="#f59e0b" /> :
                 activeTodos.length === 0 ? <LoadingRows message="No personal to-dos — use ➕ To-Do on Gmail or Calendar items below ✓" color="#15803d" /> :
                 activeTodos.slice(0, 5).map(t => <TodoItem key={t.id} item={t} onToggle={handleToggleTodo} onDelete={handleDeleteTodo} syncStatus={todoSyncStatus[t.id]} sourceConfig={personalSourceConfig} accentColor="#8b5cf6" />)}
                {activeTodos.length > 5 && <p style={{ color: '#94a3b8', fontSize: '12px', textAlign: 'center', margin: '8px 0 0' }}>+{activeTodos.length - 5} more — see Personal tab</p>}
              </div>
            </div>
            {/* Calendar */}
            <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
              <div style={{ padding: '16px 20px 4px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <GBadge size={28} /><h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '18px', color: '#0f172a', margin: 0 }}>📅 Calendar</h2>
                {!calendarLoading && !calendarError && <span style={{ fontSize: '11px', color: '#64748b', marginLeft: 'auto' }}>{calendarData.totalCount} event{calendarData.totalCount !== 1 ? 's' : ''} · personal calendar</span>}
              </div>
              <div style={{ padding: '0 16px 16px' }}>
                {calendarLoading ? <LoadingRows message="Loading calendar…" color="#0369a1" /> : calendarError ? (
                  ...
                  <>
                    <p style={{ margin: '8px 0 6px', fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Today</p>
                    {visibleCalToday.length === 0 ? <p style={{ color: '#94a3b8', fontSize: '12px', margin: '0 0 12px 4px' }}>No personal events today</p> :
                     visibleCalToday.map(e => <CalendarEventRow key={e.id} event={e} addingTodoFor={addingTodoFor} onOpenTodo={handleOpenTodo} onSaveTodo={handleSaveTodoFromCard} onCancelTodo={handleCancelTodo} />)}
                    <p style={{ margin: '12px 0 6px', fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Tomorrow — {tomorrowLabel}</p>
                    {visibleCalTomorrow.length === 0 ? <p style={{ color: '#94a3b8', fontSize: '12px', margin: '0 0 4px 4px' }}>No personal events tomorrow</p> :
                     visibleCalTomorrow.map(e => <CalendarEventRow key={e.id} event={e} addingTodoFor={addingTodoFor} onOpenTodo={handleOpenTodo} onSaveTodo={handleSaveTodoFromCard} onCancelTodo={handleCancelTodo} />)}
                    <p style={{ color: '#94a3b8', fontSize: '11px', margin: '10px 0 0 4px', fontStyle: 'italic' }}>ℹ️ Personal Google Calendar only — NIH work meetings are in Outlook</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <GBadge size={28} /><h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '18px', color: '#0f172a', margin: 0 }}>📧 Gmail</h2>
                {!gmailLoading && !gmailError && <span style={{ fontSize: '11px', color: '#64748b', marginLeft: 'auto' }}>{visibleGmailEmails.length} showing · {visibleActionedCount} need action · last 14 days</span>}
              </div>
              {gmailLoading ? <LoadingRows message="Loading Gmail…" color="#ea4335" /> : gmailError ? (
                ...
                <>
                  {visibleActionedCount > 0 && <div style={{ background: '#fffbeb', borderRadius: '8px', padding: '8px 12px', border: '1px solid #fde68a', marginBottom: '10px' }}><p style={{ margin: 0, fontSize: '12px', fontWeight: 600, color: '#92400e' }}>⚡ {visibleActionedCount} email{visibleActionedCount !== 1 ? 's' : ''} need your attention — shown first</p></div>}
                  {visibleGmailEmails.map(email => <GmailRow key={email.id} email={email} addingTodoFor={addingTodoFor} onOpenTodo={handleOpenTodo} onSaveTodo={handleSaveTodoFromCard} onCancelTodo={handleCancelTodo} onDismiss={() => handleDismissGmail(email.id)} />)}
                </>
              )}
            </div>
            {/* Slack */}
            <div style={{ background: '#fff', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <GBadge size={28} /><h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '18px', color: '#0f172a', margin: 0 }}>💬 Slack</h2>
                {!slackLoading && !slackError && <span style={{ fontSize: '11px', color: '#64748b', marginLeft: 'auto' }}>{slackMentions.length} mention{slackMentions.length !== 1 ? 's' : ''} · last 7 days</span>}
              </div>
              {slackLoading ? <LoadingRows message="Loading Slack mentions…" /> : slackError ? (
                <div style={{ background: '#fef2f2', borderRadius: '10px', padding: '14px', border: '1px solid #fecaca' }}>
        ...
        {activeView === 'jira' && (
          <div>
            <div style={{ background: 'rgba(30,41,59,0.4)', borderRadius: '12px', padding: '14px 16px', marginBottom: '12px', border: '1px solid rgba(71,85,105,0.4)', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {['All', 'CTDC', 'ICDC'].map(f => (
                <button key={f} onClick={() => setJiraFilter(f)} style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, border: '1px solid', cursor: 'pointer', background: jiraFilter === f ? '#fff' : 'transparent', color: jiraFilter === f ? '#0f172a' : '#94a3b8', borderColor: jiraFilter === f ? '#fff' : 'rgba(71,85,105,0.4)' }}>{f === 'CTDC' ? '🟢 ' : f === 'ICDC' ? '🔵 ' : ''}{f}</button>
              ))}
              <span style={{ color: 'rgba(71,85,105,0.6)', fontSize: '12px' }}>|</span>
              {['All', 'Critical', 'Major', 'Minor'].map(f => (
                <button key={f} onClick={() => setJiraPriorityFilter(f)} style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, border: '1px solid', cursor: 'pointer', background: jiraPriorityFilter === f ? '#fff' : 'transparent', color: jiraPriorityFilter === f ? '#0f172a' : '#94a3b8', borderColor: jiraPriorityFilter === f ? '#fff' : 'rgba(71,85,105,0.4)' }}>{f}</button>
              ))}
            </div>
            {['All', 'CTDC'].includes(jiraFilter) && (
              <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9', marginBottom: '12px' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <GBadge size={28} /><span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#22c55e', display: 'inline-block' }} />
                  ...
                  <span style={{ background: '#f0fdf4', color: '#15803d', borderRadius: '10px', padding: '2px 8px', fontSize: '11px', fontWeight: 700, border: '1px solid #bbf7d0', marginLeft: 'auto' }}>{jiraLoading ? '…' : filteredJira.filter(t => t.product === 'CTDC').length} open</span>
                </div>
            ...
            {['All', 'ICDC'].includes(jiraFilter) && (
              <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9', marginBottom: '12px' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <GBadge size={28} /><span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#3b82f6', display: 'inline-block' }} />
                  <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '18px', color: '#0f172a', margin: 0 }}>ICDC — Integrated Canine Data Commons</h2>
                  <span style={{ background: '#eff6ff', color: '#1d4ed8', borderRadius: '10px', padding: '2px 8px', fontSize: '11px', fontWeight: 700, border: '1px solid #bfdbfe', marginLeft: 'auto' }}>{jiraLoading ? '…' : filteredJira.filter(t => t.product === 'ICDC').length} open</span>
                </div>
                <div style={{ padding: '12px 16px' }}>
                  {jiraLoading ? <LoadingRows message="Loading from Jira…" /> : jiraError ? <LoadingRows message="Could not load Jira tasks." color="#ef4444" /> :
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
              ...
              <div style={{ padding: '12px 16px' }}>
                ...
                {workTodosLoading ? <LoadingRows message="Loading work to-dos…" color="#10b981" /> :
                 activeWorkTodos.length === 0 ? <p style={{ color: '#94a3b8', fontSize: '13px', textAlign: 'center', padding: '20px 0' }}>No work to-dos yet — use ➕ Work To-Do on Jira, Asana, or Slack items, or add one above ✓</p> :
                 activeWorkTodos.map(t => <TodoItem key={t.id} item={t} onToggle={handleToggleWorkTodo} onDelete={handleDeleteWorkTodo} syncStatus={workTodoSyncStatus[t.id]} sourceConfig={workSourceConfig} accentColor="#10b981" />)}
                {completedWorkTodos.length > 0 && (
                  <div style={{ marginTop: '16px' }}>
                    <button onClick={() => setShowWorkCompleted(!showWorkCompleted)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ color: '#94a3b8', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{showWorkCompleted ? '▲' : '▼'} Completed ({completedWorkTodos.length})</span>
                    </button>
                    {showWorkCompleted && <div style={{ marginTop: '8px' }}>{completedWorkTodos.map(t => <TodoItem key={t.id} item={t} onToggle={handleToggleWorkTodo} onDelete={handleDeleteWorkTodo} syncStatus={workTodoSyncStatus[t.id]} sourceConfig={workSourceConfig} accentColor="#10b981" />)}</div>}
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
            ...
            <div style={{ padding: '12px 16px' }}>
              ...
              {todosLoading ? <LoadingRows message="Loading to-dos…" /> :
               activeTodos.length === 0 ? <p style={{ color: '#94a3b8', fontSize: '13px', textAlign: 'center', padding: '20px 0' }}>No personal to-dos yet — use ➕ To-Do on Gmail or Calendar items, or add one above ✓</p> :
               activeTodos.map(t => <TodoItem key={t.id} item={t} onToggle={handleToggleTodo} onDelete={handleDeleteTodo} syncStatus={todoSyncStatus[t.id]} sourceConfig={personalSourceConfig} accentColor="#8b5cf6" />)}
              {completedTodos.length > 0 && (
                <div style={{ marginTop: '16px' }}>
                  <button onClick={() => setShowCompleted(!showCompleted)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ color: '#94a3b8', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{showCompleted ? '▲' : '▼'} Completed ({completedTodos.length})</span>
                  </button>
                  {showCompleted && <div style={{ marginTop: '8px' }}>{completedTodos.map(t => <TodoItem key={t.id} item={t} onToggle={handleToggleTodo} onDelete={handleDeleteTodo} syncStatus={todoSyncStatus[t.id]} sourceConfig={personalSourceConfig} accentColor="#8b5cf6" />)}</div>}
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid #f1f5f9' }}>
            ...
            <div style={{ padding: '12px 16px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                ...
                <button onClick={addGrocery} style={{ padding: '9px 18px', borderRadius: '8px', background: 'linear-gradient(135deg, #3b82f6 0%, #6366f1 100%)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>Add</button>
              </div>
            </div>
          </div>
        )}
